...
const createAccountStore = () => {
    ...
    const createUser = async(username: string, password: string, pfp: string, wallpaper: string) => {
        ...
        sessionStorage.setItem('graphite-session', JSON.stringify({userId: newUser.id}));
    };
    ...
    const updateUser = async(userId: string, data: Partial<{
        ...
        update((users) => {
            ...
            if(data.wallpaper) {
                userToUpdate.wallpaper = data.wallpaper;
            }
