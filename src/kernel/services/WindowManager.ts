...
export class WindowManager {
  ...
  subscribe(callback: Subscription): () => void {
    ...
    callback(this.windows); // Immediately send the current state
    return () => {
  ...
  public close(windowId: string): void {
    ...
    // If there are other windows, focus the top one
  ...
  public focus(windowId: string): void {
    ...
    // If it's not already focused, bring it to the front
  ...
  public toggleMaximize(windowId: string): void {
    ...
    // If we are maximizing, we must un-minimize
